...
sub attach_model_card{
	...
	print $file "*************\n";
  print $file "*****NMOS model card********\n";
	print $file ".include '$ptm_model_card'\n";
  print $file "*****PMOS model card********\n";
...
sub attach_simulation_netlist{
	...
	attach_input_vector($file, $trans_input_vec_file_name, $iteration, $time_unit, $supply, $margin, $benchmark_name);
...
sub attach_input_vector{
	...
	while( $row = <TRANS_IN_VEC_FH> ){
		...
		# print $file "V$port_name $port_name gnd pwl $slow_time$time_unit 0v";
		print $file "V$port_name $port_name gnd pwl 1$time_unit 0v";
        ### for big core ###
        # if ($bench == "c432") {
		#     print $file "V$port_name $port_name gnd pwl 1$time_unit 0v";
        # } elsif ($bench == "c864") {
             # if ($port_name =~ /N2.+/) {
		     #     print $file "V$port_name $port_name gnd pwl $margin$time_unit 0v";
             # } else {
		     #     print $file "V$port_name $port_name gnd pwl 1$time_unit 0v";
             # }
        # }
		...
		foreach $current_value (@vec_line){
            ...
            ### for big core ###
            # if ($bench == "c432") {
			    # $input_change_time = $input_change_time + 3;
            # } elsif ($bench == "c864") { 
                 # if ($port_name =~ /N2.+/) {
			     #     $input_change_time = $i*$iteration + $margin;
                 # }
            # }
            ###################
...
sub attach_analysis{
	...
	while( $row = <TRANS_IN_VEC_FH> ){
		...
		foreach $current_value (@vec_line){
            ...
            ### for big core ###
            # if ($bench == "c432") {
			     # $input_ready_time = $input_ready_time + 3;
            # } elsif ($bench == "c864") { 
                 # if ($port_name =~ /N2.+/) {
			     #     $input_ready_time = $input_ready_time + $margin;
                 # }
            # } 
            ###################
